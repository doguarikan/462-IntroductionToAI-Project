...
// Basit ama etkili heuristic: Kendi hareket seçeneklerini maksimize et, rakibin seçeneklerini minimize et
int evaluate(t_game *game, t_player *ai, t_player *human) {
	...
	// Eğer AI hareket edemezse, kaybetmiş demektir
	if (ai_moves == 0)
		return -10000;
	// Eğer insan hareket edemezse, AI kazanmış demektir
	if (human_moves == 0)
		return 10000;
	
	// Basit heuristic: Hareket özgürlüğü farkı
	return (ai_moves * 10) - (human_moves * 10);
}
...
int minimax_alpha_beta(t_game *game, t_player *ai, t_player *human, int depth, int alpha, int beta, int maximizing, char *best_move, int *best_remove_x, int *best_remove_y) {
	// Terminal durumları veya maksimum derinlik
	if (depth == 0) {
		return evaluate(game, ai, human);
	}
	
	t_player *current_player = maximizing ? ai : human;
	t_player *opponent = maximizing ? human : ai;
	
	// Hareket edebilecek yer var mı kontrol et
	int move_count;
	get_valid_move_directions(game, current_player, opponent, NULL, &move_count);
	if (move_count == 0) {
		// Oyun bitti, bu oyuncu hareket edemiyor
	...
	if (maximizing) {
		...
		for (int m = 0; m < move_cnt; m++) {
			...
			for (int r = 0; r < remove_cnt; r++) {
				...
				if (eval > max_eval) {
					max_eval = eval;
					if (depth == 3 && best_move && best_remove_x && best_remove_y) { // İlk seviye
